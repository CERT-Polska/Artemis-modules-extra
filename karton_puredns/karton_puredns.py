...
class PurednsModule(ArtemisBase):
    ...
    def process_domain_list(self, task: Task) -> None:
        ...
        Sends results as a new DOMAIN_LIST task.
        """
        domains = task.get_payload(TaskType.DOMAIN_LIST)
        ...
        try:
            ...
            # 7. Cleanup temporary files
            for f in [temp_domain_list_file, output_file, wildcards_file, massdns_file]:
                if f and hasattr(f, 'name') and os.path.exists(f.name):
                    try:
                        os.remove(f.name)
                        self.log.debug(f"Removed temporary file: {f.name}")
